...
//* all members in previous version.
//******************************************************************************
members.each {
...
//* Migrate Binding files if there are any
...
def outputMigrateAndCommitScript(def currentVersion, def datasets, def langs)
{
     ...
     if (selectionCriteria == 'LANG')
     {
         langs.each { lang ->
             ...
             if (ext)
                 migrationScriptFile << '$DBB_HOME/migration/bin/migrate.sh -r $repo ' << '-o $outputFile' << " -m MappingRule[hlq:${tempHlq}.${currentVersion}.${proj}.${group},toLower:true,extension:$ext,tagFile:" << '$tagFile]' <<" $lang" << '\n'
             else
                 migrationScriptFile << '$DBB_HOME/migration/bin/migrate.sh -r $repo ' << '-o $outputFile' << " -m MappingRule[hlq:${tempHlq}.${currentVersion}.${proj}.${group},toLower:true,tagFile:" << '$tagFile]' <<" $lang" << '\n'                    
         }
     }
...
def parseLine(def line)
{
    ...
    def version = lessHlq.minus('.' + lessVersion)
