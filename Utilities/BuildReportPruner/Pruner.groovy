...
def checkGroupName(String name) {
	def groups = store.listBuildResultGroups()
...
def deleteBR(buildRes) {
	...
	if (!buildRes.getProperty("save").equals("yes")) {
		...
		if (!preview) {
			store.deleteBuildResult(buildRes)
		}
	}
	else {
...
// will delete the builds in a group, optionally keep a specified amount of builds
...
if (opts.a) {
	...
	def groups = store.listBuildResultGroups()
	groups.each{ group ->
		def builds = store.getBuildResults(group)
	...
	if (opts.c || opts.e || opts.w) {
		...
		if (opts.c) {
			...
			def currGroup = store.getBuildResults(queryParms)
	...
	if (opts.l) {
		...
		def buildResult = store.getBuildResult(groupName, opts.l)
		deleteBR(buildResult)
		System.exit(0)
	}
	...
	if (option.equals("-a") || option.equals("--all")) {
		...
		if (selection.equals("yes") || selection.equals("YES") || selection.equals("Yes") || selection.equals("y")) {
			...
			def buildResult = store.getBuildResult(groupName, labelName)
			deleteBR(buildResult)
			System.exit(0)
		}
