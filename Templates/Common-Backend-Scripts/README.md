...
-b `<branch>`| (Deprecated) Name of the **git branch** turning into a segment of the directory path for the location within the artifact repository for the `build` pipelineType. Please switch to '-R' as the package identifier.
...
-R `<packageIdentifier>` | **Package identifier** to indicate the origin of the package. In case of a Release candidate, the release identifier is such as `rel-1.0.0`, with a `rel-`prefix followed by a version number. This is usually a computed value based on the pipeline templates. It can be automatically computed using the [computeReleaseVersion.sh](#computereleaseversionsh) script. In case of a preliminary build (pipelineType `build`), the value for packageIdentifier is the branch name used for building artifacts stored in the package.
