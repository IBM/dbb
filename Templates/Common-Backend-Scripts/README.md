...
-R `<releaseIdentifier>` | **Package identifier** to indicate the origin of the package. In case of a Release candidate, the release identifier such as `rel-1.0.0`. This is usually a computed value based on the pipeline templates. It can be automatically computed using the [computeReleaseVersion.sh](#computereleaseversionsh) script. In case of a reliminary build (pipelineType `build`), the value for releaseIdentifier is the branch name of which the package originates from.
